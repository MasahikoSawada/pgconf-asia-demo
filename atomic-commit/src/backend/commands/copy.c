...
typedef struct CopyStateData
{
	...
	/* Information needed for transition table management. */
...
#define MAX_BUFFERED_TUPLES 1000
	...
	if (cstate->rel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE)
	{
		...
		ExecSetupPartitionTupleRouting(cstate->rel, mtstate);
		 ...
		 * tuple).  This consists of just pointers now, actual maps are
		 * allocated in ExecInitPartitionResultRelInfo() if and when needed.
		 */
		...
		/*
		 * Note that ExecInitPartitionResultRelInfo manipulates only
		 * ModifyTableState directly.
		 */
		mtstate->mt_transition_tupconv_maps = cstate->transition_tupconv_maps;
	}
