 ...
 */
void
ExecSetupPartitionTupleRouting(Relation rel, ModifyTableState *mtstate)
{
	List	   *leaf_parts;
	ListCell   *cell;
	int			i;
	...
	/*
	 * If we're initializing ModifyTableState for EXPLAIN [ANALYZE],
	 * better initialize all partitions, too.
	 */
	if (mtstate->ps.state->es_top_eflags & EXEC_FLAG_EXPLAIN_ONLY ||
		mtstate->ps.state->es_instrument)
	{
		for (i = 0; i < mtstate->mt_num_partitions; i++)
			ExecInitPartitionResultRelInfo(mtstate, i);
	}
}
...
ExecFindPartition(ModifyTableState *mtstate, TupleTableSlot *slot)
{
	...
	Relation	rel;
	PartitionDispatch  *pdarray = mtstate->mt_partition_dispatch_info,
						parent;
	 ...
	 */
	if (mtstate->resultRelInfo->ri_PartitionCheck)
		ExecPartitionCheck(mtstate->resultRelInfo, slot, estate);
	...
	while (true)
	{
		...
		else if (parent->indexes[cur_index] >= 0)
		{
			...
			parent = pdarray[-parent->indexes[cur_index]];
	}
...
ExecInitPartitionResultRelInfo(ModifyTableState *mtstate, int partidx)
{
	...
	if (mtstate->mt_partitions[partidx]->ri_FdwRoutine != NULL)
	{
		FdwRoutine	*routine = mtstate->mt_partitions[partidx]->ri_FdwRoutine;

		if (routine->BeginForeignModify != NULL)
		{
			List	   *fdw_private = (List *)
									list_nth(node->fdwPartitionPrivLists,
											 partidx);

			routine->BeginForeignModify(mtstate,
			...
			mtstate->mt_partitions[partidx]->ri_PartitionIsValid = false;
	}
	...
	if (partrel->rd_rel->relhasindex)
		ExecOpenIndices(mtstate->mt_partitions[partidx], false);
