						...
						Index rti, RangeTblEntry *rte)
{
	...
	add_paths_to_append_rel(root, rel, live_childrels, NULL);
}
...
add_paths_to_append_rel(PlannerInfo *root, RelOptInfo *rel,
						List *live_childrels, UpperPathExtraData *extra)
{
	List	   *subpaths = NIL;
	...
	if (partial_subpaths_valid)
	{
		...
		if (extra)
			create_partition_agg_paths(root, rel, NIL, partial_subpaths,
	 ...
	 */
	if (subpaths_valid)
		generate_mergeappend_paths(root, rel, rel->reltarget, live_childrels,
								   all_child_pathkeys,
								   partitioned_rels, extra);
	...
	foreach(l, all_child_outers)
	{
		...
		if (subpaths_valid)
		{
			if (extra)
				create_partition_agg_paths(root, rel, subpaths, NIL,
										   NIL, required_outer, 0,
										   partitioned_rels, extra);
			else
				add_path(rel, (Path *)
						 create_append_path(rel, subpaths, rel->reltarget,
											required_outer, 0,
											partitioned_rels));
		}
	}
						   ...
						   UpperPathExtraData *extra)
{
	...
	foreach(lcp, all_child_pathkeys)
	{
		...
		if (extra)
			create_partition_agg_paths(root, rel, startup_subpaths, NIL,
									   pathkeys, NULL, 0,
									   partitioned_rels, extra);
		else
		...
		{
			if (extra)
				create_partition_agg_paths(root, rel, total_subpaths, NIL,
										   pathkeys, NULL, 0,
										   partitioned_rels, extra);
			else
				add_path(rel, (Path *) create_merge_append_path(root,
...
set_dummy_rel_pathlist(RelOptInfo *rel)
{
	...
	add_path(rel, (Path *) create_append_path(rel, NIL, rel->reltarget, NULL,
											  0, NIL));
...
generate_partition_wise_join_paths(PlannerInfo *root, RelOptInfo *rel)
{
	...
	add_paths_to_append_rel(root, rel, live_children, NULL);
