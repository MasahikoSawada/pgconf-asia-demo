...
ExecInitModifyTable(ModifyTable *node, EState *estate, int eflags)
{
	...
	if (operation == CMD_INSERT &&
		...
		ExecSetupPartitionTupleRouting(rel, mtstate);
...
ExecEndModifyTable(ModifyTableState *node)
{
	...
	for (i = 0; i < node->mt_num_partitions; i++)
	{
		...
		if (resultRelInfo)
		{
			...
			{
				FdwRoutine *routine = resultRelInfo->ri_FdwRoutine;

				if (routine->EndForeignModify != NULL)
					routine->EndForeignModify(node->ps.state, resultRelInfo);
			}
